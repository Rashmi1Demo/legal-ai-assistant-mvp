...
class CaseAnalystAgent:
    ...
    def _build_prompt(
        ...
        requested_language = "Arabic" if language.lower() == "ar" else "English"
...
- If the requested language is Arabic, use clear Modern Standard Arabic suitable for a professional legal audience.
- Proper names may remain in their original language when translation would reduce accuracy.
...
""".strip()
        ...
        for field, expected_type in REQUIRED_FIELDS.items():
            ...
            if expected_type is str:
                normalized[field] = str(
                    value or "Not identified from the supplied material."
                ).strip()
            else:
                if value is None:
                    normalized[field] = []
    ...
    def analyze(
        ...
        # Only support the two languages used by the MVP UI.
